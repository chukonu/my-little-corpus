...
class TextField(Field):
    ...
    """
    def render(self):
        return '''<label class="col-sm-2 control-label">%s</label>
        <div class="col-sm-10">
        <input type="text" class="form-control" placeholder="Title" name="%s" value="%s" placeholder="%s" %s /></div>''' % (
            self.label,
            self.field,
            self.initial,
            self.label,
...
class BigTextField(Field):
    ...
    """
    def render(self):
        return '''<label class="col-sm-2 control-label">%s</label><div class="col-sm-10">
        <textarea class="form-control" rows="3" name="%s" placeholder="%s" %s>%s</textarea></div>''' % (
            self.label,
            self.field,
            self.label,
            "required" if self.required else "",
...
class KeyField(Field):
    ...
    def render(self):
        res = '''<label class="col-sm-2 control-label">%s</label><div class="col-sm-10">
            <select class="form-control" name="%s" %s>
        ...
        for item in self.query:
            res += row % (
                # urlsafe () / id ()
                # chose urlsafe() over id()
                # for the attribute is a Key property
                item.key.urlsafe(),
                'selected="selected"' if item.key == self.initial or self.initial == item.key.urlsafe() else "",
                item
            )
...
class DateField(Field):
    ...
    def render(self):
        return '''
        <label class="col-sm-2 control-label">%s</label><div class="col-sm-10">
        <input type="date" name="%s" value="%s" class="form-control" placeholder="" %s /></div>''' % (
            self.label,
            self.field,
            self.initial,
            "required" if self.required else ""
...
class RadioField(Field):
    def __init__(self, field, label, initial="", required=False, options=None, query=None):
        super(RadioField, self).__init__(field, label, initial, required)
        self.options = options or []
        self.query = query or []

    def render(self):
        res = '''<label class="col-sm-2 control-label">%s</label>
            <div class="col-sm-10">''' % self.label
        row = """
            <label class="radio-inline"><input type="radio" name="%s" value="%s" %s />%s</label>
        """
        for index, option in enumerate(self.options):
            res += row % (self.field, index, "checked" if self.initial == index else "", option)
        return res + "</div>"
